...
//Fetch featured products from the fakesotre API
...
export const fetchFeaturedProductsByCategory = async (category: string): Promise<Product[]> => {
    try {
...
export const fetchAllCategory = async (category: string): Promise<Product[]> => {
    try {
        const response = await fetch(`${BASE_URL}/products/${category}`)
        const data = await response.json()
        console.log(`Fetched products for category ${category}:`, data)
...
export const searchProducts = async (query: string): Promise<Product[]> => {
    try {
        ...
        return data.filter((product) => product.title.toLowerCase().includes(lowerCaseQuery) ||
